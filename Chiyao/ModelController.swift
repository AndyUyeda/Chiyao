...
class ModelController: NSObject, UIPageViewControllerDataSource {
    ...
    func loadData()
    {
        pageData = UserDefaults.standard.stringArray(forKey: "vocab_words") ?? ["Add Word"]
        translationData.removeAll()
        
        for word in pageData
        {
            let key = word + "_translated"
            let translated = UserDefaults.standard.string(forKey: key) ?? "Translation"
            
            translationData.append(translated)
        }
    }
    ...
    func viewControllerAtIndex(_ index: Int, storyboard: UIStoryboard) -> DataViewController? {
        ...
        dataViewController.translationObject = self.translationData[index]
